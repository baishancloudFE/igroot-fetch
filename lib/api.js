...
const notifice = (type, res = {}) => {
  return notification[type]({
    message: `请求${type === 'error' ? '失败' : '成功'}`,
...
class RESTful {
  ...
  put(data) {
    return httpRequest.put(this.url, data, this._buildOptions())
      .then(res => res.json())
      .then(json => {
        if (json.code !== 0) {
          // 弹出提示框
  ...
  delete(data) {
    return httpRequest.delete(this.url, data, this._buildOptions())
      .then(res => res.json())
      .then(json => {
        if (json.code !== 0) {
          // 弹出提示框
